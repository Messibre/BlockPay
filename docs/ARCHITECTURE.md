...
## Components
- Frontend: React app using Mesh SDK for wallet connect, UTxO read, and tx building (payToContract, redeemFromScript). Uses REST for metadata, listings, and tx verification.
...
2. Client funds escrow using Mesh payToContract; submits txHash to backend.
...
- BE: Render/Railway; env-injected secrets.
- DB: Mongo Atlas.
