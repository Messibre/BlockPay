...
Note: After changing `.env` files, restart the dev servers so Vite/Node pick up the new environment variables.
...
### Step 6: Configure Deployed Escrow Contract Address

After you build and deploy the Aiken contract (using `aiken address`), copy the script address and add it to your `.env` files:
